...
namespace dsp::wg
{
  template<typename SampleT=float,size_t MaxLen=1<<15>
  struct VarDelayBranch final:public Node
  {
    VarDelayBranch(void) noexcept=default;
    void SetDelay(size_t d) noexcept { return (this->fracdl.SetDelay(d)); }
    ...
    inline SampleT Peek(void) const noexcept { return this->fracdl.PeekHead(); } // Read the sample at the head of the delay line without advancing it.
    ...
    {                                   // ----------- Propagate ----------- //
        for (size_t i=0;i<n;++i)        // For each sample to propagate...
        {
          ...
          SampleT v=fracdl.Read();           // Read the sample from the delay line.
    ...
    private:
      VarFracDelay<SampleT,MaxLen> fracdl;// The delay line object.
